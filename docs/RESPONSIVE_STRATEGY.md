...
Для вертикального Cover-арта широкая landscape-сцена сохраняет высоту
динамического viewport и использует отдельную верхнюю фокусную точку изображения.
Это не меняет пропорции исходника: Луна остаётся видимой, а `object-fit: cover`
по-прежнему заполняет экран без растяжения и полос. В landscape высотой до
`520px` фокус смещается ниже, чтобы ветви и Луна не ухудшали читаемость lockup.
Пока Cover активен, он владеет viewport целиком: прокрутка документа заблокирована,
а инертный `#main` не может показаться под фиксированной сценой в низком landscape.
