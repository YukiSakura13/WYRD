...
- локальный сервер через `python3 -m http.server 4173` не используем
- `localhost` не считаем рабочим контуром просмотра по умолчанию
- результат изменений проверяется через GitHub и удалённый просмотр проекта

Что это значит для дальнейшей работы:

- не предлагать запуск локального `http.server`, если пользователь сам этого не попросил
- не строить инструкции вокруг `localhost`
- по умолчанию считать GitHub основным способом просмотра результата
