...
| Action Button | `.ui-action` + `.wyrd-action-frame`; `.wyrd-cover-invitation`; `.reading-new-question-link` | screen-specific action classes | Cover Invitation, Secondary reading continuation and quiet unframed reading reset are canonical and share their exact Kit/runtime code |
...
| `.spread-continuation-btn.wyrd-action-frame--secondary` | Secondary Action | «Раскрыть пять карт» repeats the exact 1→3 continuation visual; it is hidden after the five-card reading. |
| `.reading-new-question-link` | Quiet/Text Action | Unframed 44px reset action beneath one- and three-card continuation; the only final action after five cards. |
