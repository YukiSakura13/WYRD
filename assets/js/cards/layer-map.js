export const CARD_LAYERS = {
  Плакальщица: "past",
  "Ночной Толкователь Теней": "past",
  "Следы Возвращения": "past",
  "Алтарь Пустоты": "past",
  Корень: "past",
  Водяной: "past",
  "Дух Потайного Хода": "past",
  Разлом: "past",
  "Древо Возрождения": "past",
  "Хранитель Нитей": "past",
  "Жертвенный Огонь": "past",
  Леший: "past",
  Выбор: "present",
  Тишина: "present",
  "Нити Между Мирами": "present",
  "Безмолвный Затворник": "present",
  "Верный Момент": "present",
  Свеча: "present",
  Туман: "present",
  "Тёмный Гость": "present",
  "Лисий Огонь": "present",
  "Ткачиха Судьбы": "present",
  "Хранитель Леса": "present",
  "Лесной Знахарь": "present",
  Круг: "present",
  "Зимний Сон": "present",
  "Искра Леса": "present",
  "Хранительница Малых Радостей": "present",
  "Странник Тихой Реки": "present",
  "Перо Ветра": "present",
  "Соловей Рассвета": "present",
  "Дух Скрытой Тропы": "present",
  "Светляк в Ночи": "present",
  "Лунные Влюблённые": "present",
  "Пламя Под Кожей": "present",
  "Поцелуй Тени": "present",
  "Зерно Урожая": "present",
  "Лесной Резчик": "present",
  "Прощающее Сердце": "present",
  "Хранительница Тихой Верности": "present",
  "Узел Живого Дела": "present",
  "Золотой Ручей": "present",
  "Прыжок Через Огонь": "present",
  "Хранительница Лунной Вуали": "present",
  "Всевидящий Плащ": "present",
  Грибница: "present",
  "Свой Костёр": "present",
  "Дом Под Корой": "present",
  "Родная Стая": "present",
  "Чужая Стая": "present",
  "Дружеский Совет": "present",
  "Скрытый Исток": "present",
  "Судьбоносная Встреча": "future",
  Заря: "future",
  "Золотой Олень": "future",
  "Чтец Знаков": "future",
  "Цветущая Ветвь": "future",
  "Страж Порога": "future",
  "Страж с Ключом": "future",
  "Вещий Ворон": "future",
  Рой: "future",
  "Хранитель Фонаря": "future",
  "Хранитель Времени": "future",
  "Вестник Чёрных Крыльев": "future",
  Русалка: "future",
  "Ткач Возвращённого Света": "future",
  "Скреплённые Ветви": "future",
  "Лестница Ветвей": "future",
  "Золотая Мера": "future",
  "Старейшина Леса": "future",
  "Знак Главного Пути": "future",
  "Венец После Бури": "future",
  "Эхо Вершин": "future",
  "Тропа Домой": "future",
};
